...
function addGamesToPage(games) {
    ...
    for (let counter = 0; counter < games.length; counter ++) {
        ...
        // about each game
        newDiv.innerHTML = ` <h1> ${games[counter].name} has ${games[counter].backers} backers.</h1>
                             <p>${games[counter].description}</p> 
                             <img class = "game-img" src= ${games[counter].img} />
        `;
...
let numberUnfunded = GAMES_JSON.filter ( (game) => {
    return (game.pledged < game.goal);
}).length;

// create a string that explains the number of unfunded games using the ternary operator
let displayStr = `A total of ${totalRaised} has been raised for ${totalGames}. Currently, ${numberUnfunded} ${numberUnfunded === 1 ? "game remains unfunded" : "games remain unfunded"}. We need your help to fund these amazing games!`;
...
const runnerUpGame = document.createElement("h1");
...
function searchBar() {
    deleteChildElements(gamesContainer);
    const search = document.getElementById("searchBar").value.toLowerCase().substring(0,3);
    //use filter to match search and games in GAMES_JSON
    let searchList= GAMES_JSON.filter ( (game) => {
        return game.name.toLowerCase().substring(0,3) === search;
